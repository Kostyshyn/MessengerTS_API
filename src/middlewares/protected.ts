...
export default function(req: express.Request, res: express.Response, next: express.NextFunction): express.NextFunction {
	const token = req.body.token || req.query.token || req.headers['x-access-token'];
	const { SECRET_AUTH_KEY } = process.env;
	if (token) {
		jwt.verify(token, SECRET_AUTH_KEY, (err, decoded): any => {      
			if (err) {
